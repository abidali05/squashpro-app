...
<div class="admin-page">
    ...
    <div class="admin-page-header">
        <div class="admin-page-header__left">
            <h4 class="admin-page-header__title">Contracts</h4>
            <p class="admin-page-header__subtitle">Track, sign, and manage all client contracts in one place.</p>
        </div>
    ...
    <div class="admin-filter-card">
        <span class="admin-filter-card__label">Filter Contracts</span>
        <div class="admin-filter-grid">
            ...
            <div>
                ...
                <input type="date" id="orderIdFilter" class="form-control">
            </div>
            <div class="admin-filter-grid__actions">
                <button type="button" class="admin-btn-reset" id="resetFilters">
    ...
    <div class="admin-table-card">
        <div class="admin-card-header">
            <div>
                <h5 class="admin-card-header__title">Contract List</h5>
                <p class="admin-card-header__subtitle">All contracts with current signing status and creation date.</p>
            </div>
            <span class="admin-card-header__meta">Total <strong>4</strong> contracts</span>
        </div>
                        ...
                        <div class="contract-content mb-4 p-3 border rounded bg-light">
                            ...
                            <div id="signedInfo" class="mt-4 d-none">
                                ...
                                <div class="signature-display">
                                    ...
                                    <img id="signatureImage" src="" class="img-fluid" style="max-width: 200px;">
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    $(document).ready(function () {
        $('#statusFilter, #orderIdFilter').on('change keyup', function () {
            const status  = $('#statusFilter').val().toLowerCase();
            const dateVal = $('#orderIdFilter').val().toLowerCase();
            $('#ordersTable tbody tr').each(function () {
                const rowStatus = $(this).find('td:nth-child(3)').text().toLowerCase();
                const rowDate   = $(this).find('td:nth-child(4)').text().toLowerCase();
                const sm = status  === '' || rowStatus.includes(status);
                const dm = dateVal === '' || rowDate.includes(dateVal);
                $(this).toggle(sm && dm);
            });
        });
    });

    // Contract view/sign logic
    $(document).ready(function () {
        let signaturePad;
        ...
        function updateContractStatus(data) {
            ...
            $(`[data-contract-id="${contractId}"]`).each(function () {
                ...
                row.find('.admin-badge').removeClass('admin-badge-warning').addClass('admin-badge-primary').text('Signed');
            });
            alert('Contract signed successfully!');
        }

        $('#downloadContractBtn').on('click', function () {
            window.location.href = `/contracts/${$('#contractId').text()}/download`;
        });
