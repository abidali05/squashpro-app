...
<style>
    .pool-page-black-title {
        color: #000000 !important;
    }
    .pool-page-black-link {
        color: #000000 !important;
        font-weight: 700;
        text-decoration: underline;
    }
    .pool-page-black-link:hover {
        ...
        <div class="card h-100 border shadow-xs">
            <div class="card-header bg-light border-bottom py-3">
                <h5 class="card-title mb-0 fw-bold fs-6 text-dark">
                    <i class="mdi mdi-format-list-checks text-dark me-2"></i>Pool Overview
                </h5>
            </div>
            <div class="card-body pt-3">
                <div class="detail-row py-2 border-bottom d-flex align-items-center justify-content-between">
                    ...
                    <span class="fw-semibold text-end">
                        @if($tourn)
                            <a href="{{ route('admin.tournaments.show', $tourn) }}" class="pool-page-black-link">{{ $tourn->name }}</a>
                        @else
                            —
                        @endif
                    </span>
                ...
                <div class="detail-row py-2 border-bottom d-flex align-items-center justify-content-between">
                    ...
                    <span class="badge bg-label-info fw-semibold">{{ ucfirst(($tournamentPool?->format ?: $tourn?->format) ?: 'Standard') }}</span>
                </div>
                <div class="detail-row py-2 border-bottom d-flex align-items-center justify-content-between">
                ...
                <div class="detail-row py-2 border-bottom d-flex align-items-center justify-content-between">
                    ...
                    <span>
                        @if($tournamentPool?->has_pools)
                            <span class="badge bg-label-success px-2 py-1 fw-semibold"><i class="mdi mdi-check-circle me-1"></i>Has Pools</span>
                        @else
                            <span class="badge bg-label-secondary px-2 py-1 fw-semibold"><i class="mdi mdi-minus-circle me-1"></i>No Pools</span>
                        @endif
                    </span>
                </div>
                <div class="detail-row py-2 border-bottom d-flex align-items-center justify-content-between">
                ...
                <div class="detail-row py-2 border-bottom d-flex align-items-center justify-content-between">
                    ...
                    <span class="text-dark fw-medium small">{{ $tournamentPool?->created_at ? $tournamentPool->created_at->format('Y-m-d H:i') : '—' }}</span>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <div class="col-md-6">
                        <div class="card h-100 border shadow-xs">
                            <div class="card-header bg-label-primary d-flex align-items-center justify-content-between py-2.5">
                                ...
                                <span class="badge bg-dark text-white">Index #{{ $p['pool_index'] ?? $loop->iteration }}</span>
                            </div>
                            <div class="card-body pt-3">
                                    ...
                                    <div class="list-group list-group-flush rounded border">
                                        @foreach($p['club_ids'] as $cid)
                                            ...
                                            <div class="list-group-item px-3 py-2 d-flex align-items-center gap-2">
                                                ...
                                                @else
                                                    <div class="rounded-circle bg-label-secondary d-flex align-items-center justify-content-center text-dark"
            ...
            <div class="card h-100 border shadow-xs">
                <div class="card-body text-center py-5">
                    <div class="mx-auto mb-3 rounded-circle d-flex align-items-center justify-content-center bg-light border"
                        ...
                        <i class="mdi mdi-format-list-checks text-dark" style="font-size: 32px;"></i>
                    </div>
